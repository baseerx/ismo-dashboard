...
// Main menus retired from the sidebar. Their `main_menu` rows and their routes
// still exist, so any group the rights API returns under these names is
// filtered out — otherwise assigning the rights again would quietly restore
// the menu.
const RETIRED_MAIN_MENUS = ["WF Activity Management"];
...
const userRights = async () => {
    ...
    try {
      ...
      const transformed = _(data)
        // Drop retired menus before grouping — filtering after groupBy would
        // turn the keyed dictionary into an array and the group key below
        // would silently become a numeric index.
        .reject((row: any) => RETIRED_MAIN_MENUS.includes(row.mainmenu))
        .groupBy("mainmenu")
        .map((items, mainmenu) => {
          const icon =
      ...
      // If the API fails, fall back to the static nav.
      setTransformedNavItems(navItems);
    }
  };
